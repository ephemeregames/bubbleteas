...
function Wave() {
    ...
    var DETAIL = Math.round( WIDTH / 20 ); // The number of particles used to build up the wave
    var WATER_DENSITY = 4.07;
    ...
    /** Colors settings */
    var COLOR_WATER_0 = '#FFFFFF';
    var COLOR_WATER_1 = '#FFFFFF';
    var COLOR_BUBBLE = '#000000';

    /** Bubble settings */
    var TWEETS_FREQUENCY = 400; // Milliseconds between tweet bubbles being added to the wave
    var MAX_BUBBLES = 30; // The maximum number of bubbles visible before FIFO is applied
     ...
     * Constructor.
     */
    this.initialize = function( selector, colorWater0, colorWater1, colorBubble ) {
        canvas = $(selector)[0];

        COLOR_WATER_0 = colorWater0
        COLOR_WATER_1 = colorWater1

        if (colorBubble) { COLOR_BUBBLE = colorBubble }
        ...
        if (canvas && canvas.getContext) {
            ...
            $(selector).mouseup(MouseUp);
            $(selector).bind('resize', ResizeCanvas);
    ...
    function TimeUpdate(e) {
        ...
        var gradientFill = context.createLinearGradient(WIDTH*.5,HEIGHT*.4,WIDTH*.5,HEIGHT);
        gradientFill.addColorStop(0, COLOR_WATER_1);
        gradientFill.addColorStop(1, rgbaText(COLOR_WATER_0, 1));
        ...
        gradientFill.addColorStop(0, COLOR_BUBBLE);
        gradientFill.addColorStop(1, rgbaText(COLOR_BUBBLE, 0));
    ...
    function rgbaText(h, opacity) { return 'rgba(' + hexToR(h) + ',' + hexToG(h) + ',' + hexToB(h) + ',' + opacity + ')' }
}
